...
<aside class="fixed top-0 left-0 h-screen w-64 z-50 flex flex-col bg-slate-950 border-r border-white/[0.05]"
    ...
    <nav class="relative flex-1 overflow-y-auto px-3 py-4 space-y-0.5">
        ...
        <a href="{{ route('admin.users') }}"
            ...
            {{-- Count badge --}}
            @php $userCount = \App\Models\User::where('role','!=','admin')->count(); @endphp
            <span class="text-[9px] font-bold px-1.5 py-0.5 rounded-md
                         {{ request()->routeIs('admin.users') ? 'bg-sky-400/20 text-sky-300' : 'bg-white/[0.05] text-slate-600' }}">
                {{ $userCount }}
            </span>
        </a>
    ...
    <div class="relative flex-shrink-0 border-t border-white/[0.05] p-3">
        <div class="flex items-center gap-3 px-2 py-2 rounded-xl hover:bg-white/[0.03] transition-colors duration-200 group cursor-default">
            {{-- Avatar --}}
        ...
        {{-- Logout --}}
        <form method="POST" action="{{ route('user.logout') }}" class="mt-1">
            @csrf
            <button type="submit"
